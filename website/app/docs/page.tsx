...
const toolGroups = [
  ...
  },
  {
    ...
    tools: [
      { name: "dual_network_guide", description: "Generates exact macOS or Windows steps for keeping internet and Control Hub Wi-Fi active simultaneously. Choose USB tethering, Bluetooth tethering, or a travel-router Wi-Fi-client bridge; the result includes route tests and the final adb command." },
      { name: "network_diagnostics", description: "Tests the robot ADB port and internet path concurrently, lists every active non-loopback IPv4 interface, and recommends the next routing fix when only one connection works." },
  ...
  return <main className="docsPage">
    ...
    <div className="docsLayout shell">
      ...
      <article className="docsContent">
        ...
        <h2>1. Add the MCP server</h2><h3>Codex</h3><pre><code><span>$</span> codex mcp add ftc -- npx -y ftc-mcp</code></pre><h3>Claude Code</h3><pre><code><span>$</span> claude mcp add ftc -- npx -y ftc-mcp</code></pre>
        ...
        <h2 id="tools">Complete tool reference</h2><p>All 34 MCP tools are listed below. These descriptions closely mirror the metadata the LLM receives when it decides which tool to call.</p>
        ...
        <h2 id="networking">Keep the AI and robot online</h2><p>Keep Wi-Fi connected to the Control Hub and send internet traffic through a second interface. The cheapest portable option is a phone tether. Ask the AI to call <code>dual_network_guide</code> for tailored steps, then <code>network_diagnostics</code> to verify both paths before <code>adb_connect</code>.</p><div className="callout"><b>Development only</b><p>Disconnect programming computers and auxiliary wireless equipment before a match, and follow the current FTC competition manual.</p></div><h3>macOS: phone USB tether + Control Hub Wi-Fi</h3><pre><code><span>$</span> route -n get 192.168.43.1{`\n`}<span>$</span> route -n get default{`\n`}<span>$</span> nc -vz 192.168.43.1 5555{`\n`}<span>$</span> nc -vz api.openai.com 443{`\n`}<span>$</span> adb connect 192.168.43.1:5555</code></pre><h3>Windows: phone USB tether + Control Hub Wi-Fi</h3><p>Run these in PowerShell. iPhone users need Apple Devices or iTunes so Windows exposes the USB network adapter.</p><pre><code><span>&gt;</span> Test-NetConnection 192.168.43.1 -Port 5555{`\n`}<span>&gt;</span> Test-NetConnection api.openai.com -Port 443{`\n`}<span>&gt;</span> Get-NetIPInterface -AddressFamily IPv4 | Sort-Object InterfaceMetric{`\n`}<span>&gt;</span> adb connect 192.168.43.1:5555</code></pre><p>If ADB works but the AI loses internet, lower the tether adapter’s interface metric so it becomes the preferred default route. Do not bridge or modify the Control Hub network. Bluetooth tethering is cable-free but slower; a travel router in Wi-Fi-client mode is the most reliable cable-light alternative.</p>
