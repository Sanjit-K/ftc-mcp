...
const toolGroups = [
  ...
  },
  {
    ...
    tools: [
      ...
      { name: "open_autonomous_studio", description: "Starts Autonomous Studio on 127.0.0.1, optionally loads an existing autonomous Java file, and automatically imports public commands from local subsystem and automation folders. Nothing is uploaded or hosted remotely." },
      { name: "get_autonomous_studio_draft", description: "Returns the live Studio spec together with the original imported Java. The LLM uses both to integrate timeline, action, or path-structure edits while preserving imports, annotations, subsystem initialization, lifecycle logic, helper routines, and state conditions." },
    ],
  },
  {
  ...
  return <main className="docsPage">
    ...
    <div className="docsLayout shell">
      ...
      <article className="docsContent">
        ...
        <h2 id="tools">Complete tool reference</h2><p>All 38 MCP tools are listed below. These descriptions closely mirror the metadata the LLM receives when it decides which tool to call.</p>
        ...
        <h2 id="autonomous-studio">Local Autonomous Studio</h2><p>Ask your agent to run <code>open_autonomous_studio</code>, or use <code>npx ftc-toolchain studio ~/FtcRobotController</code>. The editor binds only to <code>127.0.0.1</code>, stops with the local toolchain process, and never uploads robot code. It autosaves locally, accepts <code>.pp</code> and <code>.ftcauto.json</code>, and imports public methods from <code>subsystems/</code> and <code>automations/</code> into grouped action folders.</p><p>For imported Java, <strong>Update Java safely</strong> edits the original path builders and starting pose instead of generating a replacement class. Imports, annotations, subsystem construction, lifecycle methods, loop logic, state conditions, action implementations, and custom path-call arguments remain intact. Structural or timeline edits are handed to the agent through <code>get_autonomous_studio_draft</code>, which supplies both the live visual spec and original Java for a context-aware refactor.</p>
