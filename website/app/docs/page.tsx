  ...
  return <main className="docsPage">
    ...
    <div className="docsLayout shell">
      ...
      <article className="docsContent">
        ...
        <h2 id="networking">Automatic Wi-Fi deployment</h2><p>No phone tether is needed. Build on internet Wi-Fi, then let a local job switch to the Control Hub, deploy, and return to the original network. The AI may disconnect briefly, but the worker keeps running without internet and saves its result locally.</p><div className="callout"><b>One-time preparation</b><p>Join the Control Hub manually once so macOS or Windows saves its SSID and password. Then reconnect to your normal internet Wi-Fi.</p></div><h3>Preview without changing anything</h3><blockquote>“Call wifi_deploy_start for robot SSID FTC-12345 with dryRun true.”</blockquote><h3>Build, switch, deploy, and return</h3><blockquote>“Deploy to the saved Wi-Fi network FTC-12345.”</blockquote><p><code>wifi_deploy_start</code> builds first, returns its job ID, waits 10 seconds, switches networks, deploys through ADB, and restores the original Wi-Fi in a recovery block. After the internet connection returns, <code>wifi_deploy_status</code> reads the result. The workflow is the same on macOS and Windows; Windows uses its saved WLAN profile and macOS uses its remembered network.</p><div className="callout"><b>Development only</b><p>Disconnect programming computers before a match and follow the current FTC competition manual.</p></div>
