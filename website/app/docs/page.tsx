...
const tools = ["inspect_project", "check_project_hygiene", "create_project", "list_generated_files", "list_backups", "restore_backup", "list_samples", "search_docs", "reference_status", "update_references", "create_subsystem", "create_teleop", "hardware_manifest", "validate_hardware", "robot_status", "restart_robot_controller", "build", "build_and_deploy", "clear_robot_logs", "robot_logs", "document_subsystem"];
  ...
  return <main className="docsPage">
    ...
    <div className="docsLayout shell">
      ...
      <article className="docsContent">
        ...
        <h2 id="tools">Tool reference</h2><p>The server exposes 32 focused tools. These are the ones you’ll meet first.</p><div className="toolList">{tools.map((tool,i)=><div key={tool}><code>{tool}</code><span>{i < 6 ? "Project setup, hygiene, inventory, and recovery" : i < 10 ? "Search and maintain FTC/Pedro references" : i < 14 ? "Generate and validate structured robot code" : i < 20 ? "Inspect, build, install, and debug" : "Maintain the robot knowledge base"}</span></div>)}</div>
