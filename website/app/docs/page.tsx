...
const tools = ["setup_project", "inspect_project", "search_knowledge", "create_subsystem", "create_teleop", "create_test_opmode", "build_project", "deploy_robot", "read_logcat", "document_robot"];
  ...
  return <main className="docsPage">
    ...
    <div className="docsLayout shell">
      ...
      <article className="docsContent">
        ...
        <h2 id="first-build">3. Ask for a mechanism</h2><blockquote>“Create an intake subsystem with one motor called intakeMotor. Add spinIn, spinOut, and stop, then build the project.”</blockquote><p>The AI creates the class, updates the robot documentation, runs Gradle, and reports any compiler errors with the file and line that caused them.</p>
        <h2 id="tools">Tool reference</h2><p>The server exposes 20 focused tools. These are the ones you’ll meet first.</p><div className="toolList">{tools.map((tool,i)=><div key={tool}><code>{tool}</code><span>{i < 2 ? "Project setup and inspection" : i === 2 ? "Search FTC and Pedro references" : i < 6 ? "Generate structured robot code" : i < 9 ? "Build, install, and debug" : "Maintain the robot knowledge base"}</span></div>)}</div>
